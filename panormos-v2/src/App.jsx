  ...
  return <div>
    ...
    <div style={{marginBottom:20}}>
      <div onClick={handleExportClients} style={{
        ...
        padding:"12px 24px", background:T.bgCard, border:`1px solid ${T.border}`, borderRadius:12,
        cursor:"pointer", width:"fit-content",
      }}>
        <span style={{fontSize:16}}>📊</span>
        <span style={{fontSize:12,fontWeight:600,color:T.textSecondary}}>Excel'e Aktar</span>
      </div>
    </div>
...
// TASKS PAGE
// ─────────────────────────────────────────────
function TasksPage({tasks,setTasks,clients,staff,isAdmin}) {
  ...
  return <div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
      ...
      <div style={{display:"flex",gap:8}}>
        {isAdmin && <Btn onClick={handleExportTasks}>📊 Excel'e aktar</Btn>}
        {isAdmin && <Btn variant="primary" onClick={()=>{setModal(true);setForm({title:"",client:clients[0]?.name||"",assignee:staff[0]?.initials||"",type:"Tasarım",priority:"mid",due:""});}}>+ Görev ekle</Btn>}
      </div>
    </div>
  ...
  return <div style={{display:"flex",height:"100vh",background:T.bg,color:T.textPrimary,fontFamily:"'Inter',-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif",minHeight:700}}>
    ...
    <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
      ...
      <div style={{flex:1,overflow:"auto",padding:28}}>
        ...
        {page==="tasks"&&<TasksPage tasks={visibleTasks} setTasks={setTasks} clients={clients} staff={staff} isAdmin={isAdmin}/>}
